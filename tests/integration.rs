...
#[test]
fn can_evaluate_stacked_if_with_adder_circuit() { // this test is flackey, no valid wire in conversion table sometimes
    let gate_gen = HalfGatesGateGen::new();
    let mut garbler = Garbler::new(gate_gen);
    let mut evaluator = HalfGatesEvaluator::new();
    let mut circuit_builder = CircuitBuilder::new();
...
#[test]
fn can_evaluate_stacked_if_with_unequal_len_subcircuits() {
    let gate_gen = HalfGatesGateGen::new();
    let mut garbler = Garbler::new(gate_gen);
    let mut evaluator = HalfGatesEvaluator::new();
    let mut circuit_builder = CircuitBuilder::new();
    ...
    circuit_builder.build_stacked_if(&is_equal, &mut short_block, &mut long_block);
    let circuit_build = circuit_builder.get_circuit_build();
