...
use gc_machine::ot::ot_finite_field::{self};
use gc_machine::ot::ot_finite_field::encrypt;
...
fn can_compare_a_bit_using_std_yao() {
    ...
    let oblivious_keypair = ot_finite_field::ObliviousKeyPair::new(&pp);
    ...
    let e_label_received_from_ot = ot_finite_field::decrypt(&pp, &real_keypair.get_secret_key(), &ciphertext_wj_1);
...
fn can_evaluate_or_circuit() {
    ...
    let gate_gen = PointAndPermuteGates::new(wire_gen.clone());
    ...
    let keypair_real = ot_finite_field::RealKeyPair::new(&evaluator.get_pp());
    let pk_real = keypair_real.get_public_key();
    let sk_real = keypair_real.get_secret_key();
    let pk_oblivious = ot_finite_field::ObliviousKeyPair::new(&evaluator.get_pp()).get_public_key();
...
fn can_evaluate_is_equal_circuit() {
    ...
    let gate_gen = OriginalGates::new(wire_gen.clone());
    let mut garbler = Garbler::new(gate_gen, wire_gen);
