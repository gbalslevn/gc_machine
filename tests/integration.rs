...
fn can_evaluate_or_circuit() {
    ...
    let evaluator_input_choices = vec![[(pk_oblivious.clone(), pp.clone()), (pk_real.clone(), pp.clone())], [(pk_oblivious.clone(), pp.clone()), (pk_real, pp.clone())]]; // Eval has choosen to get bit 1. Needs to send 2 times as he needs two 1 bits for the OR gate of input AND and XOR. Even though the OR gate abstracts it to seeing it as 1 bit. The input should be the same. 
    let evaluator_decrypt_choices = vec![((sk_real.clone(), pp.clone()), 1 as u8), ((sk_real.clone(), pp.clone()), 1 as u8)]; // chooses bit 1
...
fn can_evaluate_xnor_circuit() {
    ...
    let evaluator_input_choices = vec![[(pk_oblivious.clone(), pp.clone()), (pk_real.clone(), pp.clone())]]; // Eval has choosen to get bit 1. 
    let evaluator_decrypt_choices = vec![((sk_real.clone(), pp.clone()), 1 as u8)]; // chooses bit 1
    let (circuit, wi_inputs, wj_inputs, conversion_data) = garbler.create_circuit(&circuit_build, &garbler_input_choices, evaluator_input_choices);
    let result = PointAndPermuteEvaluator::evaluate_circuit(&circuit, &wi_inputs, &wj_inputs, &conversion_data, evaluator_decrypt_choices);
    ...
    let evaluator_input_choices = vec![[(pk_real.clone(), pp.clone()), (pk_oblivious.clone(), pp.clone())]]; // Eval has choosen to get bit 0. 
    let evaluator_decrypt_choices = vec![((sk_real.clone(), pp.clone()), 0 as u8)]; // chooses bit 0
    let (circuit, wi_inputs, wj_inputs, conversion_data) = garbler.create_circuit(&circuit_build, &garbler_input_choices, evaluator_input_choices);
    let result = PointAndPermuteEvaluator::evaluate_circuit(&circuit, &wi_inputs, &wj_inputs, &conversion_data, evaluator_decrypt_choices);
...
fn can_evaluate_is_equal_circuit() {
    ...
    let (evaluator_circuit_input_a, evaluator_decrypted_input_a) = OriginalEvaluator::create_circuit_input(&a, required_bits);    
    let (circuit, wi_inputs, wj_inputs, conversion_data) = garbler.create_circuit(&circuit_build, &garbler_circuit_input_a, evaluator_circuit_input_a);

    let result = OriginalEvaluator::evaluate_circuit(&circuit, &wi_inputs, &wj_inputs, &conversion_data, evaluator_decrypted_input_a);
    assert!(result == 1);
    
    // Testing a != b, eval holds b
    let (evaluator_circuit_input_b, evaluator_decrypted_input_b) = OriginalEvaluator::create_circuit_input(&b, required_bits);    
    ...
    let result = OriginalEvaluator::evaluate_circuit(&circuit, &wi_inputs, &wj_inputs, &conversion_data, evaluator_decrypted_input_b);
    assert!(result == 0);
}