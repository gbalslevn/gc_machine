...
use crate::{
    ...
    ot::ot_finite_field::{self, CipherText, PublicKey, PublicParameters, SecretKey},
};
...
pub trait Evaluator {
    ...
    fn create_circuit_input(
        ...
        for i in 0..required_bits {
            let keypair_real = ot_finite_field::RealKeyPair::new(&self.get_pp());
            let pk_real = keypair_real.get_public_key();
            let sk_real = keypair_real.get_secret_key();
            let pk_oblivious = ot_finite_field::ObliviousKeyPair::new(&self.get_pp()).get_public_key();
...
fn decrypt_wj_input(
    ...
    let wj = ot_finite_field::decrypt(&pp, &secret_key, wj_ct);
    wj
}
